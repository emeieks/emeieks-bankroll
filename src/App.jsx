                    ...
                    <div style={{marginTop:14,marginBottom:8,borderRadius:14,overflow:"hidden",border:"1px solid rgba(96,165,250,.2)",background:"linear-gradient(135deg,rgba(14,20,38,.98),rgba(8,14,26,.99))"}}>
                      ...
                      <div style={{padding:"11px 14px",display:"flex",alignItems:"center",gap:12}}>
                        ...
                        {(()=>{
                          const src=getAvatarSrc(form.autoInfo);
                          if(src)return(
                            <div style={{width:72,height:72,flexShrink:0,position:"relative",display:"flex",alignItems:"flex-end",justifyContent:"center",overflow:"hidden"}}>
                          ...
                          return(
                            <div style={{width:72,height:72,borderRadius:10,flexShrink:0,background:"linear-gradient(135deg,rgba(124,58,237,.25),rgba(59,130,246,.15))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:700,color:"#a78bfa",textTransform:"uppercase",boxShadow:"0 0 16px rgba(124,58,237,.3)"}}>
                              {(form.autoInfo?.name||form.player).charAt(0)}
                            </div>
                          );
                        })()}
                        {/* Contenu */}
                        <div style={{flex:1,minWidth:0}}>
                          {/* Ligne 1 : joueur + sélection */}
                          <div style={{display:"flex",alignItems:"baseline",gap:7,flexWrap:"wrap",marginBottom:6}}>
                            <span style={{fontSize:15,fontWeight:700,color:"#eef3ff",textTransform:"capitalize",letterSpacing:-.2}}>{form.autoInfo?.name||form.player}</span>
                            <span style={{fontSize:13,fontWeight:500,color:"#7a9cc4"}}>{form.overUnder&&form.description?form.overUnder+" "+form.description.replace(/^(Over|Under)\s/,""):form.description}</span>
                          </div>
                          {/* Ligne 2 : cote · mise · gain */}
                          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                            <div style={{display:"flex",alignItems:"center",gap:10}}>
                              <div style={{display:"flex",flexDirection:"column",gap:1}}>
                                <span style={{fontSize:9,color:"#3d4d62",fontWeight:500,letterSpacing:.4}}>COTE</span>
                                <span style={{fontSize:13,fontWeight:600,color:"#c8d8f0"}}>@{form.odds}</span>
                              </div>
                              <span style={{width:1,height:24,background:"rgba(255,255,255,.06)",flexShrink:0}}/>
                              <div style={{display:"flex",flexDirection:"column",gap:1}}>
                                <span style={{fontSize:9,color:"#3d4d62",fontWeight:500,letterSpacing:.4}}>MISE</span>
                                <span style={{fontSize:13,fontWeight:600,color:"#c8d8f0"}}>{form.stake}€</span>
                              </div>
                              {form.bookmaker&&<><span style={{width:1,height:24,background:"rgba(255,255,255,.06)",flexShrink:0}}/><span style={{fontSize:11,color:"#3d4d62"}}>{form.bookmaker}</span></>}
                            </div>
                            {gain&&(
                              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:1}}>
                                <span style={{fontSize:9,color:"#3d4d62",fontWeight:500,letterSpacing:.4}}>GAIN POTENTIEL</span>
                                <span style={{fontSize:14,fontWeight:700,color:"#22c55e"}}>+{gain}€</span>
                              </div>
                            )}
                          </div>
                        </div>
