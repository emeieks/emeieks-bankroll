...
async function readClipboardImage(){
 ...
 if(navigator.clipboard&&navigator.clipboard.read){
  let items=[];
  try{items=await navigator.clipboard.read({unsanitized:["image/svg+xml","text/html"]});}
  catch(e){try{items=await navigator.clipboard.read();}catch(e2){items=[];}}
  for(const it of items){
   ...
   if(!htmlSrc&&it.types.includes("text/html")){
    try{const h=await (await it.getType("text/html")).text();const m=h.match(/<img[^>]+src=["']([^"']+)["']/i);if(m)htmlSrc=m[1].replace(/&amp;/g,"&");
     else if(/<svg[\s>]/i.test(h)){const sv=h.slice(h.search(/<svg[\s>]/i),h.toLowerCase().lastIndexOf("</svg>")+6);return{blob:new Blob([sv],{type:"image/svg+xml"}),type:"image/svg+xml"};}}catch(e){}
   }
 ...
 // Dernier recours : le navigateur refuse de lire (ex. logo SVG copié depuis HLTV) → on attend un Ctrl+V / Coller
 return await waitPasteEvent();
}
function waitPasteEvent(){
 return new Promise((res,rej)=>{
  const tip=document.createElement("div");
  tip.textContent="Appuie maintenant sur Ctrl+V (ou ⌘+V) pour coller l'image";
  Object.assign(tip.style,{position:"fixed",left:"50%",top:"20px",transform:"translateX(-50%)",zIndex:99999,background:"#7C3AED",color:"#fff",
   padding:"12px 18px",borderRadius:"14px",fontWeight:"700",fontSize:"14px",boxShadow:"0 8px 30px rgba(0,0,0,.5)"});
  document.body.appendChild(tip);
  const done=(fn,v)=>{clearTimeout(t);window.removeEventListener("paste",on,true);tip.remove();fn(v);};
  const on=async ev=>{
   ev.preventDefault();
   const dt=ev.clipboardData;if(!dt)return done(rej,new Error("Aucune image dans le presse-papiers"));
   const f=[...dt.files].find(x=>/image/.test(x.type));
   if(f)return done(res,{blob:f,type:f.type||"image/png"});
   const h=dt.getData("text/html")||"";const m=h.match(/<img[^>]+src=["']([^"']+)["']/i);
   if(m)return done(res,{url:m[1].replace(/&amp;/g,"&")});
   const tx=(dt.getData("text/plain")||"").trim();
   if(/^https?:\/\//.test(tx))return done(res,{url:tx});
   done(rej,new Error("Aucune image dans le presse-papiers"));
  };
  window.addEventListener("paste",on,true);
  const t=setTimeout(()=>done(rej,new Error("Aucune image collée (délai dépassé)")),15000);
 });
}

// Colle une image du presse-papiers → Supabase Storage → URL publique
// SVG → PNG 512px (Supabase Storage et les <img> gèrent mieux le PNG)
async function svgToPng(blob){
 const u=URL.createObjectURL(blob);
 try{
  const img=await new Promise((ok,ko)=>{const i=new Image();i.onload=()=>ok(i);i.onerror=ko;i.src=u;});
  const w=img.naturalWidth||512,h=img.naturalHeight||512,k=512/Math.max(w,h);
...
async function clipboardImageToUrl(folder){
 ...
 if(r.blob&&/svg/.test(r.type)){try{r.blob=await svgToPng(r.blob);r.type="image/png";}catch(e){}}
 if(r.blob)return await supaUploadPhoto(new File([r.blob],"image."+((r.type.split("/")[1]||"png").replace(/\+.*/,"")),{type:r.type}),folder);
 try{return await supaUploadPhotoFromUrl(r.url,folder);}catch(e){return r.url;}
}

// Ligne "équipe · rôle · jeu" avec IGL / AWPer en gras blanc
function roleLine(parts,role){
 const list=parts.filter(Boolean);if(!list.length)return "—";
...
const RosterEditor=memo(function RosterEditor({players,setPlayers,allPlayers,bets=[],customClubs={},setCustomClubs,rosterOpen,setRosterOpen,rosterGame,setRosterGame,rosterLeague,setRosterLeague,rosterTeam,setRosterTeam,editP,setEditP,editPForm,setEditPForm,editPPhotoUrl,setEditPPhotoUrl,editPSaving,setEditPSaving,editTeam,setEditTeam,teamLogoUrl,setTeamLogoUrl,teamLogoSaving,setTeamLogoSaving,rosterHierarchy,showToast,teamLogos={},setTeamLogos,setBets,supaPushBets}){
 ...
 const quickTeamLogo=async(team,game)=>{
  try{
   showToast("Envoi du logo…","#A78BFA");
   const url=await clipboardImageToUrl("teams");
   setTeamLogos(prev=>({...prev,[team+"__"+game]:url}));
   await fetch(SUPA_URL+"/rest/v1/players?team=eq."+encodeURIComponent(team)+"&game=eq."+encodeURIComponent(game),{method:"PATCH",headers:{"apikey":SUPA_KEY,"Authorization":"Bearer "+SUPA_KEY,"Content-Type":"application/json"},body:JSON.stringify({team_logo_url:url})});
   setPlayers(prev=>{const n={...prev};Object.keys(n).forEach(k=>{if(n[k].team===team&&n[k].game===game)n[k]={...n[k],team_logo_url:url};});return n;});
   showToast("Logo de "+team+" remplacé","#00E676");
  }catch(e){showToast((e&&e.message)||"Collage impossible","#EF4444");}
 };
 const quickPlayerPhoto=async(p)=>{
  try{
   showToast("Envoi de la photo…","#A78BFA");
   const url=await clipboardImageToUrl("players");
   await fetch(SUPA_URL+"/rest/v1/players?id=eq."+encodeURIComponent(p.id),{method:"PATCH",headers:{"apikey":SUPA_KEY,"Authorization":"Bearer "+SUPA_KEY,"Content-Type":"application/json"},body:JSON.stringify({photo_url:url,avatar_url:url})});
   setPlayers(prev=>{const n={...prev};const k=(p.name||"").toLowerCase().trim();if(n[k])n[k]={...n[k],photo_url:url,avatar_url:url};return n;});
   showToast("Photo de "+p.name+" remplacée","#00E676");
  }catch(e){showToast((e&&e.message)||"Collage impossible","#EF4444");}
 };
 ...
 const filteredPlayerKeys=searchQ.length>0
  ? Object.keys(players).filter(k=>{
     const p=players[k];
     if(rosterGame&&p.game!==rosterGame)return false;
 ...
 return(
  <div style={{marginBottom:8}}>
   ...
   <div style={{...SV_BODY,display:secOpen?undefined:"none"}}>
     ...
     {/* Search results */}
     {searchQ.length>0&&(()=>{
      const all=(filteredPlayerKeys||[]).map(k=>players[k]).filter(Boolean);
      // 1) clubs · 2) joueurs dont le nom correspond · 3) joueurs des clubs trouvés
      const byName=all.filter(p=>(p.name||"").toLowerCase().includes(searchLower))
       .sort((a,b)=>((a.name||"").toLowerCase().startsWith(searchLower)?0:1)-((b.name||"").toLowerCase().startsWith(searchLower)?0:1)||(a.name||"").localeCompare(b.name||""));
      const byClub=all.filter(p=>!(p.name||"").toLowerCase().includes(searchLower))
       .sort((a,b)=>(a.team||"").localeCompare(b.team||"")||(a.game||"").localeCompare(b.game||"")||(a.name||"").localeCompare(b.name||""));
      ...
      const Sep=({t,n})=>(<div style={{display:"flex",alignItems:"center",gap:10,margin:"10px 2px 6px"}}>
       ...
       <span style={{fontSize:11,color:"#5b6478"}}>{n}</span><span style={{flex:1,height:1,background:"rgba(255,255,255,.07)"}}/></div>);
      return(
        ...
        {clubList.map(c=>(
         <button key={c.team+c.game} onClick={()=>{
          ...
          <span style={{flex:1,minWidth:0}}>
           <span style={{display:"block",fontWeight:800,fontSize:14,color:"#E5E7EB"}}>{c.team}</span>
           <span style={{fontSize:11,color:"#6B7280"}}>{[c.game,c.league,c.n+" joueur"+(c.n>1?"s":"")].filter(Boolean).join(" · ")}</span></span>
          <GameLogo game={c.game} size={18}/>
          <span style={{color:"#6B7280",fontSize:16}}>›</span>
         </button>
        ))}
        {results.length>0&&<Sep t="Joueurs" n={results.length}/>}
        {results.map(p=>{
           ...
           {!isEditing?(
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
             ...
             <div style={{flex:1}}>
              ...
              <div style={{fontSize:10,color:"#6B7280"}}>{roleLine([p.team,p.role,p.game],p.role)}</div>
             </div>
             ...
             <button onClick={e=>{e.stopPropagation();quickPlayerPhoto(p);}} title="Coller une photo (remplace l'ancienne)" style={{padding:"5px 9px",background:"rgba(96,165,250,.08)",border:"1px solid rgba(96,165,250,.25)",borderRadius:6,color:"#93c5fd",fontSize:11,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 4H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-2"/><rect x="8" y="2" width="12" height="14" rx="2"/></svg>Photo</button>
            </div>
           ):(
                  ...
                  return(
                   <div key={p.id||p.name} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 8px",background:"rgba(255,255,255,.03)",borderRadius:6,border:"1px solid rgba(255,255,255,.05)"}}>
                    ...
                    <div style={{flex:1}}>
                     ...
                     <div style={{fontSize:10,color:"#6B7280"}}>{roleLine([p.team||"Free Agent",p.role],p.role)}</div>
                    </div>
                    <button onClick={async()=>{
                ...
                <div key={p.id||p.name} style={{padding:"8px 14px",borderTop:"1px solid rgba(255,255,255,.03)"}}>
                 {!isEditing?(
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                   ...
                   <div style={{flex:1}}>
                    <div style={{fontWeight:700,fontSize:13,color:"#E5E7EB"}}>{p.name}</div>
                    <div style={{fontSize:10,color:"#6B7280"}}>{roleLine([p.role,p.league],p.role)}</div>
                   </div>
                   <button onClick={()=>openEdit(p)} style={{padding:"5px 10px",background:"rgba(167,139,250,.08)",border:"1px solid rgba(167,139,250,.2)",borderRadius:6,color:accent,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>Modifier</button>
...
function ClubsTab({bets,allPlayers,teamLogos}){
 ...
 const clubs=useMemo(()=>{
  const m={};
  bets.forEach(b=>{if(b.game!==game)return;const t=clubOfBet(b,allPlayers);if(!t)return;(m[t]=m[t]||[]).push(b);});
  return Object.entries(m).map(([t,l])=>({team:t,...psAgg(l)})).filter(x=>x.n>=3);
 },[bets,allPlayers,game]);
 const best=[...clubs].sort((a,b)=>b.profit-a.profit).filter(x=>x.profit>0).slice(0,5);
 const worst=[...clubs].sort((a,b)=>a.profit-b.profit).filter(x=>x.profit<0).slice(0,5);
 const acc=(GAME_CFG[game]||{}).accent||"#A78BFA";
 const Row=({c,i})=>{const lg=clubLogo(c.team,game,teamLogos,allPlayers);return(
  <button onClick={()=>openClubSheet(c.team,game)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"11px 12px",background:"transparent",border:"none",borderTop:i?"1px solid rgba(255,255,255,.05)":"none",cursor:"pointer",textAlign:"left",fontFamily:"Inter,sans-serif"}}>
   <span style={{width:16,fontSize:12,fontWeight:800,color:"#5b6478"}}>{i+1}</span>
   <span style={{width:34,height:34,borderRadius:9,background:"rgba(255,255,255,.04)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
    {lg?<img src={lg} alt="" style={{width:28,height:28,objectFit:"contain"}}/>:<span style={{fontSize:11,fontWeight:800,color:"#9ca3af"}}>{c.team.slice(0,3).toUpperCase()}</span>}</span>
   <span style={{flex:1,minWidth:0}}>
    <span style={{display:"block",fontSize:14,fontWeight:700,color:"#eef1f7",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.team}</span>
    <span style={{fontSize:11.5,color:"#7c8599"}}>{c.n} paris · {c.wr.toFixed(0)}% WR · ROI {c.roi.toFixed(1)}%</span></span>
   <span style={{fontSize:15,fontWeight:800,color:pcol(c.profit)}}>{money$(c.profit)}</span>
  </button>);};
 const Box=({title,list,col})=>(
  <div style={{background:"#111827",border:"1px solid #1F2937",borderRadius:14,marginBottom:12,overflow:"hidden"}}>
   <div style={{padding:"11px 14px",fontSize:12,fontWeight:800,letterSpacing:1,textTransform:"uppercase",color:col,borderBottom:"1px solid rgba(255,255,255,.05)"}}>{title}</div>
   {list.length?list.map((c,i)=><Row key={c.team} c={c} i={i}/>):<div style={{padding:14,fontSize:13,color:"#6b7280"}}>Pas assez de paris (min. 3 par club)</div>}
  </div>);
 return(<div>
  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:14}}>
   {["LoL","CS2","Valorant","Dota2"].map(g=>{const on=g===game;const a=(GAME_CFG[g]||{}).accent||"#A78BFA";return(
    <button key={g} onClick={()=>setGame(g)} style={{padding:"9px 4px",borderRadius:11,border:"1.5px solid "+(on?a:"rgba(255,255,255,.08)"),background:on?a+"1f":"transparent",color:on?a:"#9ca3af",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4,fontFamily:"Inter,sans-serif"}}>
     <GameLogo game={g} size={20}/>{g}</button>);})}
  </div>
  <Box title="Top 5 meilleurs clubs" list={best} col="#00E676"/>
  <Box title="Top 5 pires clubs" list={worst} col="#f87171"/>
  <div style={{fontSize:11.5,color:"#5b6478",lineHeight:1.5}}>Club = équipe actuelle du joueur parié. Touche un club pour voir sa fiche.</div>
 </div>);
}
function ClubSheet({team,game,bets,allPlayers,teamLogos,onClose}){
 ...
 return(
  <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:460,background:"rgba(0,0,0,.75)",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",overflowY:"auto"}}>
   <div onClick={e=>e.stopPropagation()} style={{maxWidth:760,margin:"0 auto",minHeight:"100%",background:"#0B1220",paddingBottom:40}}>
    <div style={{position:"relative",overflow:"hidden",containerType:"inline-size",background:"radial-gradient(120% 150% at 88% 30%,"+col+"55 0%,rgba(11,18,32,0) 58%),linear-gradient(180deg,#0d1830 0%,#09101f 100%)",borderBottom:"3px solid "+col}}>
    ...
    <div style={{padding:"14px 14px 0"}}>
     <div style={{display:"flex",gap:8,marginBottom:14}}>
      ...
      <Stat l="ROI" v={tot.roi.toFixed(1)+"%"} c={pcol(tot.roi)}/>
     </div>
     <div style={{fontSize:12,fontWeight:800,letterSpacing:1,textTransform:"uppercase",color:"#9ca3af",margin:"4px 2px 8px"}}>Joueurs</div>
     <div style={{background:"#111827",border:"1px solid #1F2937",borderRadius:14,overflow:"hidden"}}>
      {rows.length===0&&<div style={{padding:14,fontSize:13,color:"#6b7280"}}>Aucun joueur enregistré dans ce club.</div>}
      {rows.map((r,i)=>{const ph=r.p.photo_url||r.p.avatar_url;return(
       <button key={r.p.name} onClick={()=>{onClose();openPlayerSheet(r.p.name);}} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"10px 12px",background:"transparent",border:"none",borderTop:i?"1px solid rgba(255,255,255,.05)":"none",cursor:"pointer",textAlign:"left",fontFamily:"Inter,sans-serif"}}>
        <span style={{width:44,height:44,borderRadius:22,overflow:"hidden",background:"rgba(255,255,255,.05)",flexShrink:0,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
         {ph?<img src={ph} alt="" style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"top"}}/>:<span style={{fontSize:16,fontWeight:800,color:"#9ca3af",marginBottom:11}}>{(r.p.name||"?")[0].toUpperCase()}</span>}</span>
        <span style={{flex:1,minWidth:0}}>
         <span style={{display:"block",fontSize:15,fontWeight:800,color:"#eef1f7"}}>{capName(r.p.name)}{r.p.role&&<span style={{fontSize:11,fontWeight:700,color:"#9ca3af",marginLeft:7}}>{r.p.role}</span>}</span>
     ...
     {tours.length>0&&<>
      ...
      <div style={{background:"#111827",border:"1px solid #1F2937",borderRadius:14,overflow:"hidden"}}>
       {tours.map((r,i)=>(
        <div key={r.t} onClick={r.t!=="Sans tournoi"?()=>{onClose();openTourSheet(r.t,game);}:undefined} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderTop:i?"1px solid rgba(255,255,255,.05)":"none",cursor:r.t!=="Sans tournoi"?"pointer":"default"}}>
...
// ── FICHE TOURNOI ─────────────────────────────────────────────────────────
function tourOfBet(b,allPlayers){
 let t=b.tournament;
 if(!t&&(b.game==="LoL"||b.game==="Valorant")){t=b.league;if(!t&&allPlayers){const p=allPlayers[(b.player||"").toLowerCase().trim()];if(p&&p.league)t=p.league;}}
 return t||"Hors tournoi";
}
function openTourSheet(name,game){try{window.dispatchEvent(new CustomEvent("emeieks-open-tour",{detail:{name,game}}));}catch(e){}}
function TourSheet({name,game,bets,allPlayers,teamLogos,onClose}){
 const logo=tourLogoSrc(name);
 const acc=(GAME_CFG[game]||{}).accent||"#A78BFA";
 const col=useLogoColor(logo,acc);
 useEffect(()=>{const k=e=>{if(e.key==="Escape")onClose();};window.addEventListener("keydown",k);return()=>window.removeEventListener("keydown",k);},[onClose]);
 const mine=useMemo(()=>bets.filter(b=>b.game===game&&tourOfBet(b,allPlayers)===name),[bets,game,name,allPlayers]);
 const tot=psAgg(mine);
 const pending=mine.filter(b=>b.status==="pending").length;
 const isHS=b=>!!b.isHeadshot||/headshot/i.test(b.description||"");
 const grp=(fn,list=mine)=>{const m={};list.forEach(b=>{const k=fn(b);if(k==null)return;(m[k]=m[k]||[]).push(b);});return Object.entries(m).map(([k,l])=>({k,l,...psAgg(l)})).filter(r=>r.n>0);};
 const lineK=b=>{const m=/(\d+(?:\.\d+)?)\s+(Kills|Headshots)/i.exec(b.description||"");return m?m[1]+" "+(/head/i.test(m[2])?"Headshots":"Kills"):null;};
 const players=grp(b=>b.player?capName(b.player):null).sort((a,b)=>b.profit-a.profit);
 const clubs=grp(b=>clubOfBet(b,allPlayers)).sort((a,b)=>b.profit-a.profit);
 const maps=grp(b=>b.mapTag||"Sans map").sort((a,b)=>a.k.localeCompare(b.k));
 const kl=grp(b=>{const k=lineK(b);return k&&!/Head/.test(k)?k:null;}).sort((a,b)=>parseFloat(a.k)-parseFloat(b.k));
 const hl=grp(b=>{const k=lineK(b);return k&&/Head/.test(k)?k:null;}).sort((a,b)=>parseFloat(a.k)-parseFloat(b.k));
 const Pair=({title,a,b,la,lb,ca,cb})=>{const A=psAgg(a),B=psAgg(b);if(!A.n&&!B.n)return null;const box=(l,x,c)=>(
  <div style={{flex:1,minWidth:0,background:"#0f1524",border:"1px solid rgba(255,255,255,.07)",borderRadius:14,padding:"12px 13px"}}>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}><span style={{fontSize:13,fontWeight:800,color:c}}>{l}</span><span style={{fontSize:11,color:"#6b7280"}}>{x.n} paris</span></div>
   <div style={{fontSize:22,fontWeight:900,color:x.n?pcol(x.profit):"#4b5563",marginTop:6}}>{x.n?money$(x.profit):"—"}</div>
   <div style={{fontSize:11.5,color:"#7c8599",marginTop:2}}>{x.n?x.wr.toFixed(0)+"% · ROI "+(x.roi>=0?"+":"")+x.roi.toFixed(1)+"%":"Aucun pari"}</div>
  </div>);
  return(<div style={{marginBottom:14}}>
   <div style={{fontSize:12,fontWeight:800,letterSpacing:1,textTransform:"uppercase",color:"#9ca3af",margin:"4px 2px 8px"}}>{title}</div>
   <div style={{display:"flex",gap:8}}>{box(la,A,ca)}{box(lb,B,cb)}</div></div>);};
 const List=({title,rows,onPick,logoOf,limit=8})=>{const[all,setAll]=useState(false);if(!rows.length)return null;const shown=all?rows:rows.slice(0,limit);return(
  <div style={{marginBottom:14}}>
   <div style={{fontSize:12,fontWeight:800,letterSpacing:1,textTransform:"uppercase",color:"#9ca3af",margin:"4px 2px 8px"}}>{title}</div>
   <div style={{background:"#111827",border:"1px solid #1F2937",borderRadius:14,overflow:"hidden"}}>
    {shown.map((r,i)=>{const lg=logoOf&&logoOf(r.k);return(
     <div key={r.k} onClick={onPick?()=>onPick(r.k):undefined} style={{display:"flex",alignItems:"center",gap:11,padding:"10px 13px",borderTop:i?"1px solid rgba(255,255,255,.05)":"none",cursor:onPick?"pointer":"default"}}>
      {logoOf&&<span style={{width:28,height:28,borderRadius:8,background:"rgba(255,255,255,.04)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,overflow:"hidden"}}>{lg?<img src={lg} alt="" style={{width:24,height:24,objectFit:"contain"}}/>:<span style={{fontSize:10,fontWeight:800,color:"#9ca3af"}}>{r.k.slice(0,3).toUpperCase()}</span>}</span>}
      <span style={{flex:1,minWidth:0}}>
       <span style={{display:"block",fontSize:14,fontWeight:700,color:"#eef1f7",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.k}</span>
       <span style={{fontSize:11.5,color:"#7c8599"}}>{r.n} paris · {r.won}V {r.lost}D · {r.wr.toFixed(0)}% · ROI {(r.roi>=0?"+":"")+r.roi.toFixed(1)}%</span>
      </span>
      <span style={{fontSize:15,fontWeight:800,color:pcol(r.profit)}}>{money$(r.profit)}</span>
     </div>);})}
    {rows.length>limit&&<button onClick={()=>setAll(v=>!v)} style={{width:"100%",padding:10,background:"transparent",border:"none",borderTop:"1px solid rgba(255,255,255,.05)",color:"#a78bfa",fontSize:12.5,fontWeight:700,cursor:"pointer"}}>{all?"Voir moins":"Voir les "+rows.length}</button>}
   </div></div>);};
 const Stat=({l,v,c})=>(<div style={{flex:1,minWidth:0,background:"#0f1524",border:"1px solid rgba(255,255,255,.07)",borderRadius:14,padding:"12px 8px",textAlign:"center"}}>
  <div style={{fontSize:11,color:"#7c8599",fontWeight:700,textTransform:"uppercase",letterSpacing:.6}}>{l}</div>
  <div style={{fontSize:19,fontWeight:900,color:c||"#eef1f7",marginTop:4}}>{v}</div></div>);
 const playerLogo=nm=>{const p=allPlayers[(nm||"").toLowerCase().trim()];return p?(p.photo_url||p.avatar_url||null):null;};
 return(
  <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:455,background:"rgba(0,0,0,.75)",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",overflowY:"auto"}}>
   <div onClick={e=>e.stopPropagation()} style={{maxWidth:760,margin:"0 auto",minHeight:"100%",background:"#0B1220",paddingBottom:40}}>
    <div style={{position:"relative",overflow:"hidden",containerType:"inline-size",background:"radial-gradient(120% 150% at 88% 30%,"+col+"55 0%,rgba(11,18,32,0) 58%),linear-gradient(180deg,#0d1830 0%,#09101f 100%)",borderBottom:"3px solid "+col}}>
     <div style={{position:"absolute",left:"2%",top:"50%",transform:"translateY(-50%)",fontSize:"min(150px, calc(96cqw / "+(Math.max(4,name.length)*0.66).toFixed(2)+"))",fontWeight:900,letterSpacing:-2,whiteSpace:"nowrap",color:"transparent",WebkitTextStroke:"1.5px "+col+"66",pointerEvents:"none",textTransform:"uppercase",lineHeight:1}}>{name}</div>
     <button onClick={onClose} aria-label="Fermer" style={{position:"absolute",top:12,left:12,zIndex:3,width:36,height:36,borderRadius:18,border:"none",background:"rgba(0,0,0,.4)",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Ic n="x" s={16} w={2.6}/></button>
     <div style={{position:"relative",zIndex:2,display:"flex",alignItems:"center",gap:16,padding:"64px 18px 24px",minHeight:190}}>
      {logo?<img src={logo} alt={name} style={{width:"clamp(76px,20vw,112px)",height:"clamp(76px,20vw,112px)",objectFit:"contain",flexShrink:0,filter:"drop-shadow(0 6px 18px rgba(0,0,0,.5))"}}/>
       :<div style={{width:90,height:90,borderRadius:22,background:"rgba(251,191,36,.12)",border:"1px solid rgba(251,191,36,.3)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fbbf24",flexShrink:0}}><Ic n="trophy" s={40}/></div>}
      <div style={{flex:1,minWidth:0}}>
       <div style={{fontSize:"clamp(24px,7vw,38px)",fontWeight:900,color:"#fff",letterSpacing:-1,lineHeight:1.05}}>{name}</div>
       <div style={{display:"flex",alignItems:"center",gap:7,marginTop:10,flexWrap:"wrap",fontSize:14,color:"#cbd5e1",fontWeight:600}}>
        <GameLogo game={game} size={18}/><span style={{color:acc}}>{game}</span>
        <span style={{color:"#5b6478"}}>•</span><span>{tot.won} - {tot.lost}</span>
        {pending>0&&<><span style={{color:"#5b6478"}}>•</span><span>{pending} en cours</span></>}
       </div>
      </div>
      {tot.n>0&&<div style={{alignSelf:"flex-end",fontSize:"clamp(26px,7vw,42px)",fontWeight:900,letterSpacing:-1,color:pcol(tot.profit),textShadow:"0 2px 14px rgba(0,0,0,.6)"}}>{money$(tot.profit)}</div>}
     </div>
    </div>
    <div style={{padding:"14px 14px 0"}}>
     <div style={{display:"flex",gap:8,marginBottom:16}}>
      <Stat l="Paris" v={tot.n}/><Stat l="Réussite" v={tot.wr.toFixed(0)+"%"}/><Stat l="ROI" v={(tot.roi>=0?"+":"")+tot.roi.toFixed(1)+"%"} c={pcol(tot.roi)}/><Stat l="Cote moy." v={tot.odds?tot.odds.toFixed(2):"—"}/>
     </div>
     <Pair title="Over / Under" a={mine.filter(b=>b.overUnder==="Over")} b={mine.filter(b=>b.overUnder==="Under")} la="Over" lb="Under" ca="#00E676" cb="#60A5FA"/>
     <Pair title="Live / Pas live" a={mine.filter(b=>b.isLive)} b={mine.filter(b=>!b.isLive)} la="Live" lb="Pas live" ca="#f87171" cb="#cbd5e1"/>
     {(game==="CS2"||game==="Valorant")&&<Pair title="Headshots / Kills" a={mine.filter(isHS)} b={mine.filter(b=>!isHS(b))} la="Headshots" lb="Kills" ca="#F472B6" cb="#A78BFA"/>}
     <List title="Joueurs" rows={players} onPick={nm=>{onClose();openPlayerSheet(nm);}} logoOf={playerLogo}/>
     <List title="Clubs" rows={clubs} onPick={t=>{onClose();openClubSheet(t,game);}} logoOf={t=>clubLogo(t,game,teamLogos,allPlayers)} limit={6}/>
     <List title="Maps" rows={maps}/>
     <List title="Lignes Kills" rows={kl} limit={10}/>
     <List title="Lignes Headshots" rows={hl} limit={10}/>
     {mine.length===0&&<div style={{padding:24,textAlign:"center",color:"#6b7280"}}>Aucun pari sur ce tournoi.</div>}
    </div>
   </div>
  </div>);
}
...
function CleanupPanel({bets,setBets,allPlayers,tourneyCal=[],showToast,onShowBets,onPlayersReload}){
 ...
 const load=async()=>{try{setRows(await cloudDb("players?select=id,name,game,team,role,league,photo_url,avatar_url,team_logo_url"));}catch(e){setRows([]);showToast&&showToast("Lecture des joueurs impossible","#EF4444");}};
 useEffect(()=>{if(open&&rows==null)load();},[open]);
 const dupGroups=useMemo(()=>{if(!rows)return [];const m={};rows.forEach(r=>{const k=(r.name||"").toLowerCase().trim()+"__"+(r.game||"");(m[k]=m[k]||[]).push(r);});return Object.values(m).filter(a=>a.length>1);},[rows]);
 const noClub=useMemo(()=>bets.filter(b=>(!b.team||b.team==="?")&&allPlayers[(b.player||"").toLowerCase().trim()]?.team),[bets,allPlayers]);
 const noTour=useMemo(()=>bets.filter(b=>tourOfBet(b,allPlayers)==="Hors tournoi"),[bets,allPlayers]);
 const calFor=b=>{const d=String(b.datetime||"").slice(0,10);if(!d)return null;const hits=(tourneyCal||[]).filter(t=>t&&t.game===b.game&&t.start&&t.end&&d>=String(t.start).slice(0,10)&&d<=String(t.end).slice(0,10));return hits.length===1?hits[0].name:null;};
 const autoTour=noTour.filter(b=>calFor(b));
 const caseMismatch=useMemo(()=>bets.filter(b=>{const p=allPlayers[(b.player||"").toLowerCase().trim()];return p&&p.name&&b.player!==p.name;}),[bets,allPlayers]);
 const merge=async grp=>{
  setBusy("dup");
  try{
   const score=r=>["team","role","league","photo_url","avatar_url","team_logo_url"].filter(k=>r[k]).length;
   const sorted=[...grp].sort((a,b)=>score(b)-score(a));const keep=sorted[0];
   for(const r of sorted.slice(1)){
    if(r.name!==keep.name)await cloudDb("bets?player=eq."+encodeURIComponent(r.name)+"&game=eq."+encodeURIComponent(r.game),{method:"PATCH",body:{player:keep.name}});
    await cloudDb("players?id=eq."+encodeURIComponent(r.id),{method:"DELETE"});
   }
   const drop=new Set(sorted.slice(1).map(r=>r.name));
   setBets(prev=>prev.map(b=>b.game===keep.game&&drop.has(b.player)?{...b,player:keep.name}:b));
   setRows(prev=>prev.filter(r=>!sorted.slice(1).some(x=>x.id===r.id)));
   showToast&&showToast(keep.name+" fusionné","#00E676");onPlayersReload&&onPlayersReload();
  }catch(e){showToast&&showToast("Erreur : "+((e&&e.message)||e).slice(0,70),"#EF4444");}
  setBusy("");
 };
 const Row=({title,sub,n,color,action,onAction,extra})=>(
  <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderTop:"1px solid "+SV.line}}>
   <span style={{minWidth:34,height:26,padding:"0 8px",borderRadius:8,background:n?color+"22":"rgba(255,255,255,.04)",color:n?color:"#4b5563",fontSize:13,fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center"}}>{n}</span>
   <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:700,color:SV.text}}>{title}</div><div style={{fontSize:12,color:SV.sub,marginTop:2}}>{sub}</div></div>
   {extra}
   {n>0&&action&&<button onClick={onAction} disabled={!!busy} style={{padding:"7px 12px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#7C3AED,#3B82F6)",color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",opacity:busy?.6:1,whiteSpace:"nowrap"}}>{action}</button>}
  </div>);
 const total=dupGroups.length+noClub.length+caseMismatch.length+autoTour.length;
 return(
  <div style={{marginBottom:8}}>
   <SuiviHead n="warn" color="#F59E0B" title="Nettoyage" sub={open?(rows==null?"Analyse…":total?total+" correction(s) possible(s)":"Tout est propre"):"Doublons, paris sans club ou sans tournoi"} open={open} onClick={()=>setOpen(v=>!v)}/>
   {open&&<div style={{...SV_BODY,padding:0}}>
    <Row n={dupGroups.length} color="#EF4444" title="Joueurs en double" sub="Même pseudo et même jeu dans Supabase (ex : ZywOo / zywoo)"/>
    {dupGroups.map(g=>(<div key={g[0].id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 14px 8px 60px",borderTop:"1px solid "+SV.line}}>
     <span style={{flex:1,fontSize:12.5,color:"#cbd5e1"}}>{g.map(r=>r.name).join(" · ")} <span style={{color:"#6b7280"}}>({g[0].game})</span></span>
     <button onClick={()=>merge(g)} disabled={!!busy} style={{padding:"5px 10px",borderRadius:8,border:"1px solid rgba(239,68,68,.35)",background:"rgba(239,68,68,.1)",color:"#f87171",fontSize:11.5,fontWeight:800,cursor:"pointer"}}>Fusionner</button>
    </div>))}
    <Row n={caseMismatch.length} color="#A78BFA" title="Paris avec un pseudo mal écrit" sub="Le nom du pari diffère de la fiche joueur (majuscules)" action="Corriger"
     onAction={()=>{setBets(prev=>prev.map(b=>{const p=allPlayers[(b.player||"").toLowerCase().trim()];return p&&p.name&&b.player!==p.name?{...b,player:p.name}:b;}));showToast&&showToast(caseMismatch.length+" pari(s) corrigé(s)","#00E676");}}/>
    <Row n={noClub.length} color="#60A5FA" title="Paris sans club" sub="On leur met le club actuel du joueur" action="Ajouter le club"
     onAction={()=>{setBets(prev=>prev.map(b=>{if(b.team&&b.team!=="?")return b;const p=allPlayers[(b.player||"").toLowerCase().trim()];return p&&p.team?{...b,team:p.team}:b;}));showToast&&showToast(noClub.length+" pari(s) mis à jour","#00E676");}}/>
    <Row n={noTour.length} color="#FBBF24" title="Paris sans tournoi" sub={autoTour.length?autoTour.length+" trouvable(s) grâce au calendrier des tournois":"À corriger à la main depuis la liste"}
     action={autoTour.length?"Assigner "+autoTour.length:null}
     onAction={()=>{const ids=new Map(autoTour.map(b=>[String(b.id),calFor(b)]));setBets(prev=>prev.map(b=>ids.has(String(b.id))?{...b,tournament:ids.get(String(b.id))}:b));showToast&&showToast(ids.size+" pari(s) assigné(s)","#00E676");}}
...
function PSStat({label,value,color,sub}){
 return(
  <div style={{flex:"1 1 0",minWidth:0,textAlign:"center",padding:"4px 2px"}}>
   <div style={{fontSize:11,fontWeight:700,color:"#8b93a7",letterSpacing:.6,textTransform:"uppercase"}}>{label}</div>
   <div style={{fontSize:22,fontWeight:800,color:color||"#f4f6fb",letterSpacing:-.6,marginTop:4,fontVariantNumeric:"tabular-nums",whiteSpace:"nowrap"}}>{value}</div>
   {sub&&<div style={{fontSize:11,color:"#6b7489",marginTop:2}}>{sub}</div>}
  </div>
 );
}

function psAgg(list){
 const s=list.filter(b=>b.status==="won"||b.status==="lost");
 const won=s.filter(b=>b.status==="won").length;
 const profit=s.reduce((a,b)=>a+(b.profit||0),0);
 const staked=s.reduce((a,b)=>a+(b.stake||0),0);
 const odds=s.length?s.reduce((a,b)=>a+(parseFloat(b.odds)||0),0)/s.length:0;
...
function PlayerSheet({name,player,bets,teamLogos,allPlayers={},activeTourneys={},onClose,onSaved,showToast}){
 ...
 const teamLogo=(p.team&&teamLogos[p.team+"__"+game])||p.team_logo_url||null;
 const photo=p.photo_url||p.avatar_url||(p.avatar_file?AVATARS_BUCKET+encodeURIComponent(p.avatar_file):null);
 const mine=useMemo(()=>bets.filter(b=>(b.player||"").toLowerCase().trim()===key).sort((a,b)=>String(b.datetime||"").localeCompare(String(a.datetime||""))),[bets,key]);
 const all=psAgg(mine);
 const cutoff=(()=>{const d=new Date();d.setDate(d.getDate()-30);return d.toISOString().slice(0,10);})();
 const last30=psAgg(mine.filter(b=>String(b.datetime||"").slice(0,10)>=cutoff));
 const pending=mine.filter(b=>b.status==="pending");
 const over=psAgg(mine.filter(b=>b.overUnder==="Over"));
 const under=psAgg(mine.filter(b=>b.overUnder==="Under"));
 const byMap={};mine.forEach(b=>{const k=b.mapTag||"Sans map";(byMap[k]=byMap[k]||[]).push(b);});
 ...
 const Spark=()=>{
  ...
  const W=300,H=56,min=Math.min(0,...pts),max=Math.max(0,...pts),rg=max-min||1;
  const x=i=>i/(pts.length-1)*W,y=v=>H-4-(v-min)/rg*(H-8);
  const d="M"+pts.map((v,i)=>x(i).toFixed(1)+","+y(v).toFixed(1)).join(" L");
  const col=pts[pts.length-1]>=0?"#00E676":"#f87171";
  return(<svg viewBox={"0 0 "+W+" "+H} width="100%" height={H} preserveAspectRatio="none" style={{display:"block"}}>
   <line x1="0" x2={W} y1={y(0)} y2={y(0)} stroke="rgba(255,255,255,.12)" strokeDasharray="3,5"/>
 ...
 const Table=({title,groups})=>{
  const rows=Object.entries(groups).map(([k,l])=>[k,psAgg(l)]).filter(([,s])=>s.n>0).sort((a,b)=>b[1].n-a[1].n);
  if(!rows.length)return null;
  return(
   <div style={{background:"#0f1524",border:"1px solid rgba(255,255,255,.06)",borderRadius:16,overflow:"hidden",marginBottom:10}}>
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 44px 54px 54px 70px",gap:6,padding:"11px 14px 8px",fontSize:11,fontWeight:700,color:"#6b7489",letterSpacing:.4}}>
     <span>{title}</span><span style={{textAlign:"right"}}>PARIS</span><span style={{textAlign:"right"}}>RÉUS.</span><span style={{textAlign:"right"}}>ROI</span><span style={{textAlign:"right"}}>PROFIT</span>
    </div>
    {rows.map(([k,s])=>(
     <div key={k} style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 44px 54px 54px 70px",gap:6,padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,.05)",fontSize:13,alignItems:"center"}}>
      <span style={{color:"#e5e7eb",fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{k}</span>
      <span style={{textAlign:"right",color:"#9ca3af"}}>{s.n}</span>
      <span style={{textAlign:"right",color:s.wr>=55?"#00E676":s.wr<45?"#f87171":"#cbd5e1",fontWeight:700}}>{s.wr.toFixed(0)}%</span>
      <span style={{textAlign:"right",color:pc(s.roi),fontWeight:600}}>{(s.roi>=0?"+":"")+s.roi.toFixed(0)}%</span>
      <span style={{textAlign:"right",color:pc(s.profit),fontWeight:800}}>{money(s.profit)}</span>
     </div>
    ))}
   </div>
  );
 ...
 return(
  <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:450,background:"rgba(0,0,0,.72)",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",overflowY:"auto",animation:"overlayIn .2s ease"}}>
   <div onClick={e=>e.stopPropagation()} style={{maxWidth:760,margin:"0 auto",minHeight:"100%",background:"#0B1220",boxShadow:"0 0 60px rgba(0,0,0,.6)"}}>
    ...
    {/* ── BANNIÈRE ── */}
    <PlayerBanner name={displayName} photo={photo} role={p.role} game={game} team={p.team} league={p.league} logo={teamLogo} profit={all.n?all.profit:null} onTeam={p.team?()=>{onClose();openClubSheet(p.team,game);}:null}>
    ...
    <div style={{padding:"14px 14px 30px"}}>
         ...
         <div style={{background:"#0f1524",border:"1px solid rgba(255,255,255,.06)",borderRadius:16,overflow:"hidden",marginBottom:10}}>
          ...
          {rows.map(([k,st])=>{const cur=k===curT;const sel=selT===k;return(
           <div key={k} onClick={()=>setPeriod(sel?"all":"t:"+k)} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",borderTop:"1px solid rgba(255,255,255,.05)",background:sel?"rgba(124,58,237,.12)":cur?"rgba(251,191,36,.05)":"transparent",cursor:"pointer"}}>
            ...
            {k!=="Hors tournoi"&&<button onClick={e=>{e.stopPropagation();onClose();openTourSheet(k,game);}} title="Fiche du tournoi" aria-label={"Fiche "+k}
             style={{width:30,height:30,borderRadius:9,border:"1px solid rgba(251,191,36,.3)",background:"rgba(251,191,36,.08)",color:"#fbbf24",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Ic n="trophy" s={14}/></button>}
           </div>);})}
         </div>);
       })()}
      </>);
     ...
     <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
      {[["Over",over,"#00E676"],["Under",under,"#60a5fa"]].map(([l,s,c])=>(
       <div key={l} style={{background:"#0f1524",border:"1px solid rgba(255,255,255,.06)",borderRadius:16,padding:"12px 14px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}><span style={{fontSize:14,fontWeight:800,color:c}}>{l}</span><span style={{fontSize:12,color:"#6b7489"}}>{s.n} paris</span></div>
        <div style={{fontSize:22,fontWeight:800,color:s.n?pc(s.profit):"#4b5366",marginTop:6}}>{s.n?money(s.profit):"–"}</div>
        <div style={{fontSize:12,color:"#8b93a7",marginTop:2}}>{s.n?s.wr.toFixed(0)+"% · ROI "+(s.roi>=0?"+":"")+s.roi.toFixed(0)+"%":"Aucun pari"}</div>
       </div>
      ))}
     </div>
     {(()=>{
      const isHS=b=>!!b.isHeadshot||/Headshot/i.test(b.description||"");
      const hasHS=game==="CS2"||mine.some(isHS);
      const hasLive=game==="LoL"||mine.some(b=>b.isLive);
      const pairs=[].concat(
       hasHS?[["Paris kills",mine.filter(b=>!isHS(b)),"#fbbf24"],["Paris headshots",mine.filter(isHS),"#818cf8"]]:[],
       hasLive?[["Live",mine.filter(b=>b.isLive),"#fb7185"],["Non-live",mine.filter(b=>!b.isLive),"#60a5fa"]]:[]);
     ...
     <div style={{background:"#0f1524",border:"1px solid rgba(255,255,255,.06)",borderRadius:16,overflow:"hidden"}}>
      <div style={{padding:"12px 14px 6px",fontSize:14,fontWeight:700,color:"#e5e7eb"}}>Derniers paris {pending.length>0&&<span style={{fontSize:12,color:"#60a5fa",fontWeight:600}}>· {pending.length} en cours</span>}</div>
      {mine.length===0&&<div style={{padding:"14px",fontSize:13,color:"#6b7489"}}>Aucun pari sur ce joueur.</div>}
      {mine.slice(0,15).map(b=>{
       const st=b.status;const col=st==="won"?"#00E676":st==="lost"?"#f87171":"#60a5fa";
       return(
        <div key={b.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,.05)",borderLeft:"3px solid "+col}}>
         <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:13,fontWeight:600,color:"#e5e7eb"}}>{b.description} {b.mapTag&&<span style={{fontSize:11,color:"#fbbf24",marginLeft:4}}>{b.mapTag}</span>}</div>
          <div style={{fontSize:12,color:"#6b7489",marginTop:2}}>{String(b.datetime||"").slice(0,10)} · @{b.odds} · {b.stake}$ · {b.bookmaker}</div>
         </div>
         <div style={{fontSize:14,fontWeight:800,color:col}}>{st==="pending"?"En cours":money(b.profit||0)}</div>
        </div>
       );
      })}
     </div>
...
function PlayerBanner({name,photo,role,game,team,league,logo,children,profit,profitSub,onTeam}){
 const acc=(GAME_CFG[game]||{}).accent||"#A78BFA";
 const col=useLogoColor(logo,acc);
 return(
  <div style={{position:"relative",overflow:"hidden",containerType:"inline-size",background:"radial-gradient(120% 150% at 88% 30%,"+col+"55 0%,rgba(11,18,32,0) 58%),linear-gradient(180deg,#0d1830 0%,#09101f 100%)",borderBottom:"3px solid "+col,transition:"background .4s"}}>
   {team&&<div style={{position:"absolute",left:"2%",top:"50%",transform:"translateY(-50%)",fontSize:"min(150px, calc(96cqw / "+(Math.max(4,String(team).length)*0.66).toFixed(2)+"))",fontWeight:900,letterSpacing:-2,whiteSpace:"nowrap",color:"transparent",WebkitTextStroke:"1.5px "+col+"66",pointerEvents:"none",textTransform:"uppercase",lineHeight:1,transition:"all .4s"}}>{team}</div>}
   {children}
   <div style={{position:"relative",zIndex:2,display:"flex",alignItems:"flex-end",gap:12,padding:"58px 14px 0",minHeight:190,maxWidth:900,margin:"0 auto"}}>
    ...
    <div style={{flex:1,minWidth:0,paddingBottom:20}}>
     ...
     <div style={{display:"flex",alignItems:"center",gap:7,marginTop:10,flexWrap:"wrap",fontSize:14,color:"#cbd5e1",fontWeight:600}}>
      ...
      <span style={{color:"#5b6478"}}>•</span>{onTeam&&team?<button onClick={onTeam} style={{background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.14)",borderRadius:8,padding:"2px 9px",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>{team} ›</button>:<span>{team||"Agent libre"}</span>}
      {league&&<><span style={{color:"#5b6478"}}>•</span><span>{league}</span></>}
     </div>
    </div>
...
function PlayerEditSheet({p,name,game:game0,teamLogo,photo,onClose,onSaved,showToast,teamLogos={},allPlayers={},onDelete,isNew}){
 ...
 const pasteImg=async(folder,k)=>{
  try{
   const r=await readClipboardImage();
   if(r.blob&&/svg/.test(r.type)){try{r.blob=await svgToPng(r.blob);r.type="image/png";}catch(e){}}
   if(r.blob){await upload(new File([r.blob],"image."+((r.type.split("/")[1]||"png").replace(/\+.*/,"")),{type:r.type}),folder,k);return;}
   set(k,r.url);
   ...
   <div onClick={e=>e.stopPropagation()} style={{maxWidth:760,margin:"0 auto",minHeight:"100%",background:"#0B1220",display:"flex",flexDirection:"column"}}>
    ...
    <PlayerBanner name={f.name} photo={f.photo} role={f.role} game={f.game} team={f.team} league={f.league} logo={f.logo}>
     <div style={{position:"absolute",top:12,left:12,right:12,display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:3}}>
      <button onClick={onClose} aria-label="Fermer" style={{width:36,height:36,borderRadius:18,border:"none",background:"rgba(0,0,0,.4)",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Ic n="x" s={16} w={2.6}/></button>
      <span style={{fontSize:12,fontWeight:700,color:"#e9e3ff",background:"rgba(0,0,0,.4)",padding:"6px 12px",borderRadius:20}}>{isNew?"Nouveau joueur · aperçu en direct":"Aperçu en direct"}</span>
     </div>
    </PlayerBanner>
    ...
    <div style={{position:"sticky",bottom:0,zIndex:5,marginTop:14,background:"rgba(9,14,28,.97)",borderTop:"1px solid rgba(255,255,255,.07)",padding:"12px 14px calc(12px + env(safe-area-inset-bottom))"}}>
     <div style={{maxWidth:732,margin:"0 auto",display:"flex",gap:8}}>
      ...
      <button onClick={save} disabled={!!busy||!f.name.trim()||(!dirty&&!isNew)}
       style={{flex:2,padding:"14px",borderRadius:14,border:"none",background:(dirty||isNew)&&f.name.trim()?"linear-gradient(135deg,#7C3AED,#3B82F6)":"rgba(255,255,255,.06)",color:(dirty||isNew)&&f.name.trim()?"#fff":"#6b7489",fontWeight:800,fontSize:15,cursor:(dirty||isNew)?"pointer":"default",fontFamily:"Inter,sans-serif",opacity:busy?.6:1}}>
       {busy==="save"?"Enregistrement…":isNew?(f.name.trim()?"Ajouter le joueur":"Entre un pseudo"):dirty?"Enregistrer les modifications":"Aucune modification"}
      </button>
     </div>
    </div>
   </div>
 ...
 *{-webkit-tap-highlight-color:transparent;}
 ...
 <span style={{display:"flex"}}><Ic n="cloud" s={13}/></span><span>{syncing?"Sync…":supaOk?"Sync":"Cloud"}</span>{pendingSync>0&&<span title="Paris en attente d'envoi" style={{marginLeft:2,padding:"1px 6px",borderRadius:8,background:"#F59E0B",color:"#111",fontSize:10,fontWeight:900}}>{pendingSync}</span>}
 ...
 {statsTab==="plus"&&(()=>{
 const Item=({icon,color,title,sub,onClick,children,danger,right})=>{const Tag=children?"label":"button";return(
  <Tag onClick={onClick} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"13px 14px",background:"transparent",border:"none",borderTop:"1px solid "+SV.line,cursor:"pointer",textAlign:"left",fontFamily:"Inter,sans-serif",boxSizing:"border-box"}}>
   <span style={{width:36,height:36,borderRadius:11,background:color+"1f",color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Ic n={icon} s={17}/></span>
   <span style={{flex:1,minWidth:0}}><span style={{display:"block",fontSize:14.5,fontWeight:700,color:danger?"#f87171":SV.text}}>{title}</span><span style={{display:"block",fontSize:12,color:SV.sub,marginTop:2}}>{sub}</span></span>
   {right||<span style={{color:"#4b5563",fontSize:18}}>›</span>}
   {children}
  </Tag>);};
 const Group=({t,children})=>(<><SuiviLabel>{t}</SuiviLabel><div style={{background:SV.card,border:"1px solid "+SV.line,borderRadius:16,overflow:"hidden",marginTop:-1}}><div style={{marginTop:-1}}>{children}</div></div></>);
 return(
 <div style={{marginBottom:16}}>
  <Group t="SAUVEGARDE">
   <Item icon="cloud" color="#22C55E" title={backingUp?"Sauvegarde en cours…":"Sauvegarde complète"} sub="Télécharge toutes tes données (paris, joueurs, réglages)" onClick={downloadBackup}/>
   <Item icon="down" color="#60A5FA" title="Exporter en CSV" sub="Pour Excel ou Google Sheets" onClick={exportCSV}/>
   <Item icon="down" color="#A78BFA" title="Exporter en JSON" sub="Copie brute de tous les paris" onClick={exportJSON}/>
   <Item icon="up" color="#F59E0B" title="Importer un JSON" sub="Ajoute des paris depuis un fichier">
    <input type="file" accept=".json" style={{display:"none"}} onChange={e=>{if(e.target.files[0])importJSON(e.target.files[0]);e.target.value="";}}/>
   </Item>
  </Group>
  <Group t="OUTILS">
   <Item icon="flask" color="#FBBF24" title="Mode Test" sub="Filtrer les stats pour tester une stratégie" onClick={()=>setTestingOpen(v=>!v)}
    right={<span style={{fontSize:11.5,fontWeight:800,color:testingOpen?"#FBBF24":"#6b7280",padding:"3px 8px",borderRadius:7,background:testingOpen?"rgba(251,191,36,.12)":"rgba(255,255,255,.04)"}}>{testingOpen?"ACTIF":"OFF"}</span>}/>
  </Group>
 </div>);
 })()}
 ...
 const Tile=({l,v,c,sub})=>(<div style={{flex:1,minWidth:0,background:"#0f1524",border:"1px solid rgba(255,255,255,.07)",borderRadius:14,padding:"12px 8px",textAlign:"center"}}>
  ...
  {sub&&<div style={{fontSize:10.5,color:"#6b7280",marginTop:2}}>{sub}</div>}</div>);
 return(
 <div style={{marginBottom:14}}>
 ...
 const roi=t.staked>0?(t.profit/t.staked*100):0;
 return(
 <div key={t.name} className="stat-row" onClick={()=>openTourSheet(t.name,game)} style={{cursor:"pointer"}}>
 <div style={{display:"flex",alignItems:"center",gap:9}}>
 ...
 {tourSheet&&<TourSheet name={tourSheet.name} game={tourSheet.game} bets={bets} allPlayers={allPlayers} teamLogos={teamLogos} onClose={()=>setTourSheet(null)}/>}
 {clubSheet&&<ClubSheet team={clubSheet.team} game={clubSheet.game} bets={bets} allPlayers={allPlayers} teamLogos={teamLogos} onClose={()=>setClubSheet(null)}/>}
 {playerSheet&&(()=>{
 ...
 {/* MODAL ADD PLAYER */}
 {modalPlayer&&(
 <PlayerEditSheet p={{name:pform.name||"",game:pform.game||"CS2",team:pform.team||"",league:pform.league||"",role:pform.role||""}}
  name={pform.name||""} game={pform.game||"CS2"} teamLogo={pform.team?(teamLogos[pform.team+"__"+(pform.game||"")]||""):""} photo="" isNew
  teamLogos={teamLogos} allPlayers={allPlayers} showToast={showToast}
  onClose={()=>setModalPlayer(false)}
  onSaved={(np,logo)=>{
   applyPlayerSaved("",np,logo);
   setModalPlayer(false);
   setPform({name:"",game:"LoL",league:"",role:"",team:""});
   if(view==="add")setForm(f=>({...f,player:np.name,autoInfo:{...np,name:np.name}}));
  }}/>
 )}
